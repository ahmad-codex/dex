...
module.exports = async function (deployer, _network, accounts) {
    ...
    await Promise.all(
        [Dai, Rep, Bat, Zrx, Dex].map(contract => deployer.deploy(contract))
    );
    const [dai, rep, bat, zrx, dex] = await Promise.all(
        [Dai, Rep, Bat, Zrx, Dex].map(contract => contract.deployed())
    );
    await Promise.all([
